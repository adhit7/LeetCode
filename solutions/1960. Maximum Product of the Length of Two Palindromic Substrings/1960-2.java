class Solution {
  public long maxProduct(String s) {
    ...
    for (int i = 1; i <= n; ++i)
      hashFromL[i] = (hashFromL[i - 1] * kBase + s.charAt(i - 1) - 'a' + 1) % kMod;

    for (int i = n - 1; i >= 0; --i)
      hashFromR[i] = (hashFromR[i + 1] * kBase + s.charAt(i) - 'a' + 1) % kMod;
  ...
  private static final long kBase = 26L;
  private static final long kMod = (long) 1e9 + 7;
  ...
  private long leftHash(int l, int r, long[] hashFromL, long[] pow) {
    long hash = (hashFromL[r] - hashFromL[l] * pow[r - l]) % kMod;
    return hash < 0 ? hash + kMod : hash;
  }

  private long rightHash(int l, int r, long[] hashFromR, long[] pow) {
    long hash = (hashFromR[l] - hashFromR[r] * pow[r - l]) % kMod;
    return hash < 0 ? hash + kMod : hash;
  }
}
