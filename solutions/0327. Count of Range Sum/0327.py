class Solution:
  def countRangeSum(self, nums: List[int], lower: int, upper: int) -> int:
    ...
    self.mergeSort(prefix, 0, n, lower, upper)
    return self.ans

  def mergeSort(self, prefix: List[int], l: int, r: int, lower: int, upper: int) -> None:
    if l >= r:
      return

    m = (l + r) // 2
    self.mergeSort(prefix, l, m, lower, upper)
    self.mergeSort(prefix, m + 1, r, lower, upper)
    self.merge(prefix, l, m, r, lower, upper)

  def merge(self, prefix: List[int], l: int, m: int, r: int, lower: int, upper: int) -> None:
